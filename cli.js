...
module.exports = {
    ...
    const questions = [
      {
        name: 'email',
        type: 'email',
        message: 'Enter your Cloudier-Site e-mail address:',
        validate: function (value) {
          if (value.length) {
            return true;
          } else {
            return 'Please enter your Cloudier-Site e-mail address.';
          }
        }
      },
      {
  ...
  displayHelp: () => {
    console.log(
      chalk.blue(
        figlet.textSync('Cloudier-Site', { horizontalLayout: 'fitted' })
      )
    );
    console.log(chalk.underline('cloudiersite-cli v1.00'));
    console.log(chalk.bold('Usage'), ': cloudiersite-cli', chalk.yellow('command'), chalk.gray('<args>'));
    ...
    return new Promise(async(resolve, reject) => {
      for (var x = 0; x < res.length; x++) {
        ...
        if (res[x].type == 'FOLDER') {
          ...
          console.log('Writing... ' + ('./' + newPath).replace(/appdrag/g, "cloudiersite"));
